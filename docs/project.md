...
Admins can use **Reports → Provider Future Availability** to select any Provider, including inactive Providers and all employment types, and read every existing schedule week ending today or later. The report shows remaining dates, full-week shift requests, completion status, and weekly notes for both draft and published periods. Overlapping periods stay separate, missing submissions are labeled, and no future date horizon is imposed. Selection is saved in the URL and Refresh retrieves the latest saved data. **PDF for All Providers** retrieves fresh reports for every listed Provider regardless of search or selection, then opens the print dialog to save one combined PDF with each Provider starting on a new page.

The report defines today using the API's `SCHEDULING_TIMEZONE` setting, which defaults to `America/New_York` and requires a valid IANA timezone. It returns and displays the effective cutoff date and timezone; stored dates are displayed without browser timezone conversion. Week data and notes are read together in a single joined database query. Unset daily availability is blank, while week-level submission and completion status remain visible. Print / Save as PDF provides a portrait report with Provider details, dates, requests, and complete notes, hiding navigation and controls. This is a read-only report with no new persistence tables. See [Provider Future Availability Report](planned-features/provider-future-availability-report.md).
...
The schedule workspace includes **Solver tuning & run history** for occasional tuning and controlled experiments. Seven bounded weights cover Provider and manager preferences, requested workload, balance, and historical fairness. Administrators can adjust a single run or explicitly save organization defaults; coverage remains fixed and Hard Constraints remain enforced.

Each accepted run records resolved settings, captured inputs, execution identity, and original outcome measurements on its Schedule Job. Historical settings are read-only. Users can reuse weights with current data or try different weights on the same captured inputs. Manual draft descendants retain solver provenance, while publication checks current eligibility. Matching-input comparisons use outcome measurements rather than ranking runs by differently weighted total scores. See [Solver Controls](planned-festures/solver-controls.md) for bounds, persistence details, and follow-up work. This feature requires migration `202609140003`.
